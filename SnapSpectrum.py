...
def process_circles(centers, hsv_image, img_path):
    ...
    with open(csv_file_path, 'a', newline='') as file:
        ...
        if not file_exists:
            header = ['Image Name', 'Circle', 'Mean Hue', 'Std Dev Hue', 'Mean Saturation', 'Std Dev Saturation'] + list(color_ranges.keys())
        ...
        for index, center in enumerate(centers):
            ...
            row = [filename, f"Circle {index+1}", f"{mean_hue:.2f}", f"{std_dev_hue:.2f}", f"{mean_saturation:.2f}", f"{std_dev_saturation:.2f}"]
